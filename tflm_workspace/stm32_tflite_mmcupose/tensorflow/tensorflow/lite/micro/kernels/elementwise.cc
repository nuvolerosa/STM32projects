...
bool IsTanhSupportedType(const TfLiteType type) {
...
TfLiteStatus TANHEval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteInt8) {
    const int8_t* in_data = GetTensorData<int8_t>(input);
    int8_t* out_data = GetTensorData<int8_t>(output);
    for (int64_t i = 0; i < num_elements; ++i) {
      const float x =
          (static_cast<int32_t>(in_data[i]) - input->params.zero_point) *
          input->params.scale;
      const float y = std::tanh(x);
      int32_t q =
          static_cast<int32_t>(y / output->params.scale + 0.5f) +
      ...
      out_data[i] = static_cast<int8_t>(q);
    }
    return kTfLiteOk;
  }

  if (input->type == kTfLiteUInt8) {
    const uint8_t* in_data = GetTensorData<uint8_t>(input);
    uint8_t* out_data = GetTensorData<uint8_t>(output);
    for (int64_t i = 0; i < num_elements; ++i) {
      const float x =
          (static_cast<int32_t>(in_data[i]) - input->params.zero_point) *
          input->params.scale;
      ...
      out_data[i] = static_cast<uint8_t>(q);
    }
    return kTfLiteOk;
  }
...
TfLiteRegistration* Register_TANH() {
  static TfLiteRegistration r = {
      ...
      /*free=*/nullptr,
      /*prepare=*/
      elementwise::GenericPrepare<elementwise::IsTanhSupportedType>,
