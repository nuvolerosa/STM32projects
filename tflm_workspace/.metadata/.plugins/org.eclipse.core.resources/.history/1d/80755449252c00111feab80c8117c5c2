...
namespace
{
    ...
    /* Usage was ~350672 bytes; keep margin in the 7 MiB SDRAM window (see STM32F746NGHX_FLASH.ld). */
    constexpr uint32_t kTensorArenaSize 	= 384 * 1024;
    /* Linked in external SDRAM (see STM32F746NGHX_FLASH.ld). NOLOAD: zero in main after SDRAM init. */
    uint8_t tensor_arena[kTensorArenaSize] __attribute__((section(".tensor_sdram"), aligned(16)));
...
int main(void)
{
    ...
    // Initialize UART1
    uart1_init();
    // Initialize LCD (also calls BSP_SDRAM_Init when DATA_IN_ExtSDRAM is not set)
    LCD_Init();
    std::memset(tensor_arena, 0, kTensorArenaSize);

    // Init variables
    // const int B = 32;   // batch dimension in your reshape
    const int J = 19;   // skeleton joints
    const int C = 3;    // (x, ?, y) where x is idx 0 and y is idx 2
    ...
    TF_LITE_REPORT_ERROR(error_reporter, "tensor_arena ptr %x (expect 0xC0......)",
  	...
  	static tflite::MicroOpResolver<42> resolver(error_reporter);
    ...
    op_status = static_cast<TfLiteStatus>(
        op_status & resolver.AddBuiltin(tflite::BuiltinOperator_SPLIT,
                                        tflite::ops::micro::Register_SPLIT(), 1, 3));
    op_status = static_cast<TfLiteStatus>(
        op_status & resolver.AddBuiltin(tflite::BuiltinOperator_LOGISTIC,
                                        tflite::ops::micro::Register_LOGISTIC(), 1, 2));
    op_status = static_cast<TfLiteStatus>(
    ...
    op_status = static_cast<TfLiteStatus>(
        op_status & resolver.AddBuiltin(tflite::BuiltinOperator_MUL,
                                        tflite::ops::micro::Register_MUL(), 1, 3));
    ...
    /* Internal DTCM/SRAM (320 KiB). */
    constexpr uintptr_t kSramStart = 0x20000000u;
    constexpr uintptr_t kSramEnd = 0x20000000u + (320u * 1024u);
    /* External SDRAM region used for tensor arena (STM32F746NGHX_FLASH.ld: SDRAM @ 0xC0100000, 7 MiB). */
    constexpr uintptr_t kSdramStart = 0xC0100000u;
    constexpr uintptr_t kSdramEnd = 0xC0100000u + 0x00700000u;
    const bool in_sram = (out_ptr >= kSramStart) && (out_ptr < kSramEnd);
    const bool in_sdram = (out_ptr >= kSdramStart) && (out_ptr < kSdramEnd);
    if (!in_sram && !in_sdram) {
    	TF_LITE_REPORT_ERROR(error_reporter, "Output data pointer not in SRAM or SDRAM, aborting read.");
    	while (1);
    }
    ...
    const int out_zp 		= model_output->params.zero_point;
    //TF_LITE_REPORT_ERROR(error_reporter, "scale = %f", out_scale);
